...
Rspec.describe DiaryEntry do
  ...
  describe "#count_words" do
    ...
    it "returns zero when contents  is empty"
  ...
  describe "#reading_time" do
    context "given a wpn of some sensible number(200)" do
      it "returns the ceiling of the number of minutes it takes to read the contents" do
        diary_entry = Diary_Entry.new("my_title", "one " * 550)
        expect(diary_entry.reading_time(200)).to eq 3
      end
    end

    context "given a wpn of 0" do 
      it "fails" do
        diary_entry = Diary_Entry.new ("my_title", "one two three")
        expect { diary_entry.reading_time(0) }.to raise_error "Reading speed must be above zero."
      end
    end  
  end

  describe "#reaing_chunk" do
